...
impl BinanceTrader {
    ...
    // ========== User Data Stream 관련 메서드 ==========

    /// User Data Stream 시작 및 이벤트 수신
    pub async fn start_user_data_stream<F>(&self, mut event_handler: F) -> Result<(), ExchangeError>
    where
        F: FnMut(UserDataEvent) + Send + 'static,
    {
        loop {
            match self.connect_user_data_stream(&mut event_handler).await {
                Ok(_) => {
    ...
    async fn connect_user_data_stream<F>(&self, event_handler: &mut F) -> Result<(), ExchangeError>
    where
        F: FnMut(UserDataEvent) + Send + 'static,
    {
        let api_key = self
